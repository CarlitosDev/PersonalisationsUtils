...
create TABLE adform.impressionsClicksMapped DISTKEY(campaign_id)  COMPOUND SORTKEY (yyyy_mm_dd, campaign_id, client_id)
AS
(
SELECT DISTINCT
...
inner join adform.meta_campaigns as tableB
  on cast(tbClicks.campaign_id as INTEGER) = tableB.campaignsid
...
matchingimpression
FROM adform.impressionsClicksMapped
WHERE campaign_id = 133118
order by yyyy_mm_dd ASC


select * from adform.meta_clients where clientsname LIKE '%Wella%'


select count(*)  from
adform.impressionsClicksMapped
where client_id = 133118
...
uk_17+q4_beamly_joop_joop wow+fathersday_xxxxxx_53124_intnt_coty